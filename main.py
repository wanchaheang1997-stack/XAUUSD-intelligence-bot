...
async def build_report(bot, chat_id, topic_id=None):
    try:
        ...
        report = (
            ...
            f"🕐 {last_candle_time}{weekend_note}\n"
            "💰 *PRICE*\n"
            f"  Current : *${price}*\n"
            f"  H1 High : ${high}  |  H1 Low : ${low}\n\n"
            "📊 *DAILY BIAS*\n"
            f"  {daily_bias}\n"
            f"  EQ Level : ${equilibrium}\n\n"
            "🔍 *1H STRUCTURE*\n"
            f"  {bos_type} - _{bos_reason}_\n"
            f"  Trend : {trend_label}\n\n"
            "📐 *KEY LEVELS*\n"
            f"  PDH : ${pdh}  |  PDL : ${pdl}  |  PDC : ${pdc}\n"
            f"  Support    : ${support}\n"
            f"  Resistance : ${resistance}\n\n"
            "🧠 *ICT CONCEPTS*\n"
            f"  Bull FVG : {fvg_bull_str}\n"
            f"  Bear FVG : {fvg_bear_str}\n"
            f"  Bull OB  : {ob_bull_str}\n"
            f"  Bear OB  : {ob_bear_str}\n\n"
            "📊 *VOLUME PROFILE*\n"
            f"  POC : ${poc}\n"
            f"  VAH : ${vah}\n"
            f"  VAL : ${val}\n\n"
            ...
            f"  MACD     : {macd_val}\n"
            f"  Signal   : {signal_val}\n"
            f"  Histogram: {hist_val} {hist_arrow}\n\n"
            "📰 *GOLD AND MACRO NEWS*\n"
            f"{news_lines}\n\n"
            "---\n"
            "_E11 Sniper Bot - Educational use only_"
...
def main():
  ...
  logger.info("Waiting 15s for old instance to shut down...")
    time.sleep(15)
    ...
    scheduler.start()
    logger.info("E11 Sniper Bot running - Reports at 08:00, 14:00, 19:00 ICT")
    logger.info("SMC Signal checker active - scanning every 1 minute")
